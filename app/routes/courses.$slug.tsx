...
// Enrollment itself is handled via the purchase confirmation page.
// This action handles star ratings from enrolled students.
...
export async function action({ params, request }: Route.ActionArgs) {
  ...
  const parsed = parseFormData(await request.formData(), rateSchema);
  if (!parsed.success) {
    return data(
      { error: parsed.errors.rating ?? "Invalid rating." },
      { status: 400 }
    );
  }
